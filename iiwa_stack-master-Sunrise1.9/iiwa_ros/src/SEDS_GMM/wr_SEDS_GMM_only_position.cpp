...
//模型初始化
  ...
  //运行SEDS模型
    int d=3;float T=1.0/20.0;float T1=1.0/1000.0; int GMMtime=0;int SEDSstage=0;
...
//       force_go_pub.publish(force_go);
    ...
    //如果距离小于某个值之后，就可以执行位置命令并给SEDSstage加一
      ...
      cout << " the mean of vol: ";
      cout << xd_mean << "; \n";
      if(xd_mean<0.0050)//距离远说明还在SEDS范围中   //sqrt(pose_comd.pose.position.x-xT[0])+sqrt(pose_comd.pose.position.z-xT[2])<-0.0000000001
      {
        SEDSstage=2;
...
//       force_go_pub.publish(force_go);
  ...
  if (SEDSstage==3)
  {
      ...
      cout << xd_mean3 << "; \n";
      if(xd_mean3<0.0025)//距离远说明还在SEDS范围中   //sqrt(pose_comd.pose.position.x-xT[0])+sqrt(pose_comd.pose.position.z-xT[2])<-0.0000000001
      {
        SEDSstage=4;
      }
  }
