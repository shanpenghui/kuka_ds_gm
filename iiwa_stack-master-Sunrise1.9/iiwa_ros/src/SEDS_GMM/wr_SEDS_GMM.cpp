...
//模型初始化
  ...
  //设定SEDS模型运算时的变量
    int d=3;float T=1.0/15.0;float T1=1.0/1000.0; int GMMtime=0;
...
//       client.call(config);
      stiff_comd.position.x=50;
      stiff_comd.position.y=50;
      stiff_comd.position.z=50;
...
//       // ROS_INFO("%f,%f,%f", kx,ky,kz);
    ...
    //如果距离小于某个值之后，就可以执行位置命令并给SEDSstage加一
      ...
      cout << " the mean of vol: ";
      cout << xd_mean << "; \n";
      if(xd_mean<0.090)//距离远说明还在SEDS范围中   //sqrt(pose_comd.pose.position.x-xT[0])+sqrt(pose_comd.pose.position.z-xT[2])<-0.0000000001
      {
        SEDSstage=2;
...
//       set_impedance(config,pt_mode);
//       client.call(config);
      stiff_comd.position.x=200;
      stiff_comd.position.y=200;
      stiff_comd.position.z=200;
      stiff_command_pub.publish(stiff_comd);
//       //ROS_INFO("%f,%f,%f", kx,ky,kz);
      // pose_go_pub.publish(pose_go);
      // force_go_pub.publish(force_go);
...
void set_impedance(iiwa_msgs::ConfigureSmartServo& config1,int& pt1)
{
 if(pt1==0&&pt_flg==0){
  ...
  config1.request.mode.cartesian_stiffness.stiffness.x = 500;
  config1.request.mode.cartesian_stiffness.stiffness.y = 500;
  config1.request.mode.cartesian_stiffness.stiffness.z = 500;
  config1.request.mode.cartesian_stiffness.stiffness.a =100;
